...
void *MFAAC_Create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	UINT32 recommendedSampleRate = MFAAC::FindBestSamplerateMatch(sampleRate);
	...
	UINT32 recommendedBitRate = MFAAC::FindBestBitrateMatch(bitrate);
	...
	auto enc = std::make_unique<MFAAC::Encoder>(encoder, bitrate, channels, sampleRate, bitsPerSample);
...
void MFAAC_GetAudioInfo(void *, struct audio_convert_info *info)
{
	...
	info->samples_per_sec = MFAAC::FindBestSamplerateMatch(info->samples_per_sec);
}
